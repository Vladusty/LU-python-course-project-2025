...
cimpulung       = City_SIRD(10000, initial_infected=0, name="Cimpulung")
cnapoca         = City_SIRD(5000, intial_infected=0, name"Cluj-Napoca")
debrecen        = City_SIRD(3000, intial_infected=0, name"Debrecen")
durres          = City_SIRD(25000, intial_infected=0, name"Durres")
...
granada         = City_SIRD(150000, intial_infected=0, name="Granada")
kyiv            = City_SIRD(20000, intial_infected=0, name="Kyiv")
lisboa          = City_SIRD(35000, intial_infected=0, name="Lisboa")
milan           = City_SIRD(120000, intial_infected=0, name="Milan")
pest            = City_SIRD(10000, intial_infected=0, name="Pest(Budapest)")
suceava         = City_SIRD(20000, intial_infected=0, name="Suceava")
seville         = City_SIRD(45000, intial_infected=0, name="Seville")
siena           = City_SIRD(50000, intial_infected=0, name="Siena")
targoviste      = City_SIRD(25000, intial_infected=0, name="Târgoviște")
toledo          = City_SIRD(42000, intial_infected=0, name="Toledo")
trnovo          = City_SIRD(35000, intial_infected=0, name="Veliko Tarnovo")
vienna          = City_SIRD(35000, intial_infected=0, name="Vienna")
zagreb          = City_SIRD(5000, intial_infected=0, name="Zagreb")
zurich          = City_SIRD(6000, intial_infected=0, name="Zurich")

fes             = City_SIRD(175000, intial_infected=0, name"Fes")
izmir           = City_SIRD(30000,  initial_infected=0, name="Izmir")
jerusalem       = City_SIRD(10000,  initial_infected=0, name="Jerusalem")
antalya         = City_SIRD(10000, initial_infected=0, name="Antalya")
...
cities = [
    ...
    rome, moscow, sofia, belgrade, lviv, cimpulung, cnapoca, debrecen, durres,
    geneve, granada, kyiv, lisboa, milan, pest, suceava, seville, siena,
    targoviste, toledo, trnovo, vienna, zagreb, zurich, fes, izmir, jerusalem,
    antalya, cologne, olomouc, rouen, yaroslavl, smolensk
]
...
G.add_edge(constantinople, antalya,      weight=1.0) 
G.add_edge(constantinople, trnovo,       weight=0.8) 
G.add_edge(constantinople, sofia,        weight=0.7)
G.add_edge(constantinople, kyiv,         weight=0.3) 
G.add_edge(constantinople, moscow,       weight=0.2)  

G.add_edge(cairo, venice,                weight=0.8)
G.add_edge(cairo, genoa,                 weight=0.7)
G.add_edge(cairo, jerusalem,             weight=0.6) 
...
G.add_edge(barcelona, valencia,          weight=1.0) 
...
G.add_edge(toulouse, granada,            weight=0.5)
...
G.add_edge(trnovo, cimpulung,            weight=0.3) 
G.add_edge(cimpulung, suceava,           weight=0.4)
...
G.add_edge(antalya, jerusalem,           weight=0.5) 
...
G.add_edge(sarai, moscow,                weight=0.5) 
G.add_edge(sarai, kyiv,                  weight=0.3) 
G.add_edge(sarai, lviv,                  weight=0.2) 
...
G.add_edge(lubeck, hamburg,              weight=1.0) 
...
G.add_edge(novgorod, riga,              weight=0.7)
...
G.add_edge(marseille, paris,             weight=0.8) 
G.add_edge(barcelona, paris,             weight=0.6) 

G.add_edge(vienna, prague,               weight=0.7) 

G.add_edge(lviv, krakow,                 weight=0.6) 

G.add_edge(paris, milan,                 weight=0.8)
G.add_edge(cologne, milan,               weight=0.8)
G.add_edge(lubeck, venice,               weight=0.8)
G.add_edge(danzig, venice,               weight=0.8)
G.add_edge(krakow, vienna,               weight=0.8)
G.add_edge(prague, venice,               weight=0.8)
...
def run_sim_with_tk(G, compute_commute_forces, super_commute_spikes, total_days=2554, step_ms=40):
    ...
    for name, default_val in DEFAULT_PARAMS.items():
        tk.Label(form, text=name).grid(row=row, column=0, sticky="w", padx=5, pady=2)
    ...
    def show_initial_infected_window(sim_params):
        ...
        for city in sorted(G.nodes, key=lambda c: c.name):
            ...
            # Noklusējums – pašreizējais I (no sākotnējā konstruktorā dotā)
    ...
    next_btn.grid(row=row+2, column=0, columnspan=2, pady=10)
