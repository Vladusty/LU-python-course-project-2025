...
def run_sim_with_tk(G, compute_commute_forces, super_commute_spikes,
    ...
    # ==== 1. SOLIS: PARAMETRU IEVADES LOGS ====
    ...
    def on_start():
        ...
        total_days   = parse_param("total_days")
        step_ms      = parse_param("step_ms")
        ...
        # Izmet parametru formu un pāriet uz kartes režīmu
        form.destroy()

        # ==== 2. SOLIS: PILNEKRĀNA KARTE ====
    ...
    start_btn = tk.Button(form, text="Sākt simulāciju", command=on_start)
    start_btn.grid(row=row+2, column=0, columnspan=2, pady=10)

    root.mainloop()


# ==== PALAIŽAM TKINTER AR PARAMETRU DIALOGU ====
